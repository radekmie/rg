...
export function inlineReachability({ edges }: ast.GameDeclaration) {
  function findThePath(start: ast.EdgeName, target: ast.EdgeName): ast.EdgeLabel[] | string {
    var x: ast.EdgeName = start;
  ...
  function substitutePath(originalEdge: ast.EdgeDeclaration, path: ast.EdgeLabel[]) {
    if (path.length == 0) {
    ...
    var last = originalEdge;
    for (const l of path.slice(1)) {
      const next = ast.EdgeDeclaration({ lhs: last.rhs, rhs: freshVar(), label: l });
      edges.push(next);
      last = next;
    }
  ...
  for (const e of edges) {
    if (e.label.kind === 'Reachability') {
      ...
      const path = findThePath(e.lhs, e.rhs);
      if(typeof path === 'object') {
        substitutePath(e, path)
      } else {
