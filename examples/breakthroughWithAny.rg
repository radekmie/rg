...
playerBegin, move: player = PlayerOrKeeper(turnPlayer);
move, set(position:Position): position != Position(null);
...
moved, playerEnd: board[pos] = pieceOfPlayer[turnPlayer];
playerEnd, endCheck: player = PlayerOrKeeper(keeper);

endCheck, playerBegin: any kEndCheck -> kContinue;
endCheck, preEnd: any kEndCheck -> kTerminate;
preEnd, end: player = PlayerOrKeeper(keeper);

kEndCheck, kWin: directionOfPlayer[turnPlayer][pos] == Position(null);
kWin, kSetScore: goals[turnPlayer] = Score(100);
kSetScore, kTerminate: goals[opponent[turnPlayer]] = Score(0);

kEndCheck, kNotWin: directionOfPlayer[turnPlayer][pos] != Position(null);
kNotWin, kCheckLose: turnPlayer = opponent[turnPlayer];
kCheckLose, kContinue: ? move -> moved;
kCheckLose, kLose: ! move -> moved;
kLose, kWin: turnPlayer = opponent[turnPlayer];
