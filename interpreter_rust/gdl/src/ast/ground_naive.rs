...
impl<Id: Clone + Ord> Game<Id> {
    pub fn ground_naive(self, distinct: &Id, or: &Id) -> Self {
        ...
        let mut subterms: Vec<_> = rules.iter().map(Rule::get_subterms).collect();
        ...
        loop {
            for i in 0..rules.len() {
                ...
                for unification in unifications {
                    ...
                    let has_failed_static_term = rule.prune_static_terms(&static_terms);
                    if !has_failed_static_term {
                        if let Some(rule) = rule.eval_distinct(distinct, or) {
            ...
            for rule in rules_to_add.drain(..) {
                ...
                subterms.insert(index, rule.get_subterms());
...
impl<Id: Clone + Ord> Rule<Id> {
    fn get_subterms(&self) -> (Vec<Term<Id>>, Vec<Term<Id>>) {
        let (mut xs, mut ys): (Vec<_>, _) = self.subterms().cloned().partition(Term::has_variable);
        xs.sort_unstable();
        ys.sort_unstable();
        (xs, ys)
    }
...
mod test {
    ...
    test!(
        ...
        goal(oPlayer, 0) :- open & ~line(o) & ~line(x)
        goal(oPlayer, 100) :- line(o)
        goal(oPlayer, 50) :- ~open & ~line(o) & ~line(x)
        goal(xPlayer, 0) :- line(o)
        goal(xPlayer, 0) :- open & ~line(o) & ~line(x)
        goal(xPlayer, 100) :- line(x)
        goal(xPlayer, 50) :- ~open & ~line(o) & ~line(x)
        index(1)
        index(2)
        index(3)
